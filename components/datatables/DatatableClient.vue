  ...
  return filterData
    ...
    .sort((a: { [x: string]: any }, b: { [x: string]: any }) => {
      ...
      const column = state.columns.find(
        (column: { name: any }) => column.name === sortingKey
      );
...
const isColumnFilterable = (key: string) => {
  const column = state.columns.find(
    (column: { name: string }) => column.name === key
  );
...
function getObjectValue<T extends object>(
  obj: T,
  path: string,
...
const renderCellContent = (item: any, column: Column) => {
  if (column.isNumber) {
    ...
    let returnData = {
      text: getObjectValue(item, column.custom.display ?? column.name),
    } as RenderColumn;
    ...
    if (column.custom.icon) {
      ...
      if (column.custom.iconColorObject) {
        returnData.iconColorObject = getObjectValue(
          item,
    ...
    if (
      ...
      getObjectValue(item, column.custom.parent.params) !=
        getObjectValue(item, column.custom.params)
    ) {
      returnData.parent = {
      ...
      if (column.custom.parent.iconColorObject) {
        returnData.parent.iconColorObject = getObjectValue(
          item,
    ...
    const generateBadge = (value: any) => ({
      class: `badge bg-${column.badge?.type || "default"}-subtle text-${
        column.badge?.textColor || "default"
      } p-2`,
    ...
    const handleBadgesForArray = (array: any[]) => {
      return array.map((arrayItem: any) =>
        generateBadge(getObjectValue(arrayItem, column.badge.display))
      );
    };
    ...
    if (Array.isArray(columnValue)) {
      ...
      // Sub-case 2: Use `badge.types` for array items
        ...
        return columnValue.flatMap((arrayItem: any) =>
          handleBadgesWithTypes(getObjectValue(arrayItem, column.badge.display))
        );
      }
    }
    ...
    return {
      ...
      props: {
        textColor:
          lightOrDark(colorValue) == "light" ? "text-dark" : "text-light",
        text: colorValue,
      },
    };
...
const generateExportData = () => {
  ...
  const data = sortedItems.value.map((item: any) => {
    return state.columns.map((column: Column) =>
      renderExportCellContent(item, column)
    );
  });
  ...
  const tableHtml = `
    <table>
      <thead>
        <tr>
          ${state.columns
            .map((column: { label: any }) => `<th>${column.label}</th>`)
            .join("")}
        </tr>
      </thead>
      <tbody>
...
const renderPrintCellContent = (item: any, column: Column) => {
  ...
  if (typeof content === "string") {
    ...
    return content
      .map(
        (badge) =>
          `<span class="${badge.class}" style="${badge.style}">${badge.text}</span>`
      )
      .join("");
  } else if (content?.component === "StackedAvatar") {
...
<template>
      ...
      <div class="col-sm-12 col-md-6">
        <div class="dataTables_length" v-if="props.table_show">
          <label>
            ...
            >
              <option
                v-for="(option, index) in state.pageSizeOptions"
                :value="option"
                :key="index"
              >
                {{ option }}
            ...
            <template v-for="(item, index) in paginatedItems" :key="index">
                ...
                >
                  <div :class="column.class">
                    ...
                    <template v-else>
                      ...
                      <span
                        v-if="
                          typeof renderCellContent(item, column) == 'string' ||
                          typeof renderCellContent(item, column) == 'number'
                        "
                      >
                        {{ renderCellContent(item, column) }}
                      </span>
                      <PartialsPercentageBadge
                        v-else-if="
                      ...
                      <UtilsStackedAvatar
                        ...
                        "
                        :collections="
                          renderCellContent(item, column).props.collections
                        "
                      />
                      <button
                      ...
                      <template
                        v-else-if="
                          Array.isArray(renderCellContent(item, column))
                        "
                      >
                        <span
                      ...
                      <span
                        v-else-if="renderCellContent(item, column)?.class"
                        :class="renderCellContent(item, column).class"
                        :style="renderCellContent(item, column).style"
                      >
                        {{ renderCellContent(item, column).text }}
                      </span>
                      <div
                      ...
                      >
                        <div
                          :style="{
                            backgroundColor: renderCellContent(item, column)
                              .text,
                          }"
                          class="p-1 rounded-2"
                        >
                          <span
                            :class="renderCellContent(item, column).textColor"
                          >
                            {{ renderCellContent(item, column).text }}
                          </span>
                        </div>
              ...
              <tr v-if="!paginatedItems.length">
                <td
                  ...
                  valign="top"
                >
                  <SpinnerComponent :loading="loadingState"></SpinnerComponent>
          ...
          <ul class="pagination">
            ...
            <template v-else>
              ...
              >
                ...
                <li v-else>
                  <span class="page-link" style="pointer-events: none"
                    >...</span
                  >
                </li>
              </template>
  ...
  <div
    ...
    class="dt-button-info"
    style="display: none"
  >
    <h2>Copy to clipboard</h2>
    <div>Copied {{ paginatedItems.length }} rows to clipboard</div>
  </div>
  <PartialsBaseOffcanvas
    title="Track Shipment"
    :fetchParams="{ param: selectedItem }"
    ref="offcanvasRef"
  />
</template>
